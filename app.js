...
const APP_VERSION = "2.1"; // Версия приложения

const EMAILJS_PUBLIC_KEY = 'el1zdjpGo0PNUArJH';
const EMAILJS_SERVICE_ID = 'service_24havuk';
const EMAILJS_TEMPLATE_ID = 'template_4tktel3';
...
async function init() {
  ...
  if (!versionValid) {
    console.log('Версия приложения устарела, требуется обновление');
    return;
  }
...
function loadSettings() {
  ...
  if (settings) {
    ...
    if (currentSettings.email === undefined) {
      ...
      currentSettings.emailVerified = false;
    }
    updateEmailStatus();
  }
...
async function generateQRCode(username) {
  try {
    ...
    await QRCode.toCanvas(qrCode, `diamondshare:user:${username}`, {
      width: 200,
      margin: 2,
      color: {
        dark: '#3b82f6',
...
function setupEventListeners() {
  ...
  document.getElementById('add-email-btn').addEventListener('click', showEmailModal);
  document.getElementById('send-email-code').addEventListener('click', sendVerificationCode);
  document.getElementById('verify-email-btn').addEventListener('click', verifyEmailCode);
...
async function loadChatsList() {
  ...
  try {
    ...
    tabsContainer.innerHTML = `
      ...
      <div class="chat-tab" data-tab="new"></div>
    `;
    chatsList.appendChild(tabsContainer);
    ...
    if (contacts.length === 0) {
      ...
      emptyState.innerHTML = `
        ...
        <p>Нажмите "+ Чат" чтобы добавить первый чат</p>
...
function showEmailModal() {
  if (currentSettings.email && currentSettings.emailVerified) {
    showAlert('Ваш email уже подтвержден: ' + currentSettings.email, 'Email подтвержден');
    return;
  }
  
  document.getElementById('email-modal').style.display = 'flex';
  document.getElementById('email-input').value = currentSettings.email || '';
  document.getElementById('email-form').style.display = 'block';
  document.getElementById('code-form').style.display = 'none';
...
function setupCodeInputs() {
  ...
  const hiddenInput = document.getElementById('email-code-input');
...
async function sendVerificationCode() {
  ...
  document.getElementById('send-email-code').textContent = 'Отправка...';
  ...
  try {
    ...
    // Сохраняем email в настройках (но пока не подтвержден)
    currentSettings.email = email;
    saveSettings();
    
  } catch (error) {
    console.error('Ошибка отправки кода:', error);
    document.getElementById('email-error').textContent = 'Ошибка отправки кода. Попробуйте позже.';
  } finally {
    document.getElementById('send-email-code').disabled = false;
    document.getElementById('send-email-code').textContent = 'Отправить код';
  }
}

function verifyEmailCode() {
  const enteredCode = document.getElementById('email-code-input').value.trim();
  
  if (enteredCode.length !== 6) {
    document.getElementById('code-error').textContent = 'Введите все 6 цифр кода';
    return;
  }
  
  if (enteredCode === emailVerificationCode) {
    currentSettings.emailVerified = true;
    saveSettings();
    updateEmailStatus();
    closeEmailModal();
    showAlert('Ваш email успешно подтвержден! Теперь вы защищены.', 'Email подтвержден');
  } else {
    document.getElementById('code-error').textContent = 'Неверный код подтверждения';
    // Подсветим неверные поля
    document.querySelectorAll('.code-input').forEach(input => {
...
function updateEmailStatus() {
  ...
  if (currentSettings.email && currentSettings.emailVerified) {
    emailStatus.textContent = currentSettings.email;
    emailStatus.classList.add('verified');
  } else if (currentSettings.email) {
    emailStatus.textContent = currentSettings.email + ' (не подтвержден)';
    emailStatus.classList.remove('verified');
  } else {
    emailStatus.textContent = 'Добавить почту';
    emailStatus.classList.remove('verified');
  }
}
